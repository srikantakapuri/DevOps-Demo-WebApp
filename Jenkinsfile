pipeline {
    ...
    stage('Deploy To Test') {
           ...
           deploy adapters: [tomcat8(url: 'http://18.219.153.31:8080/', credentialsId: 'tomcat', path: '' )], contextPath: '/QAWebapp', war: '**/*.war'
 ...
 stage ('parallel') {
    ...
    //}
    	...
    	stage("deploy to Prod"){
    		steps{
    		   deploy adapters: [tomcat8(credentialsId: 'tomcat', path: '', url: 'http://18.222.249.168:8080/')], contextPath: '/ProdWebapp', war: '**/*.war'
